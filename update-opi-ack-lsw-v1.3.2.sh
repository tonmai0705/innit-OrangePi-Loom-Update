...
updJsontmp=$HOME/setting/updatetmp.json
opiMajor=$(echo "$vs" | cut -d'.' -f1)
opiMinor=$(echo "$vs" | cut -d'.' -f2)
opiPatch=$(echo "$vs" | cut -d'.' -f3)
opiDate=$(jq '.date' "$updJson")
month=${opiDate:4:2}
day=${opiDate:6:2}

while true; do
if ping -c 1 github.com &> /dev/null; then
#url for Github Update
vsGit=$(curl -s https://raw.githubusercontent.com/tonmai0705/innit-OrangePi-Loom-Update/refs/heads/Develop/version.txt)
gitMajor=$(echo "$vsGit" | cut -d'.' -f1)
gitMinor=$(echo "$vsGit" | cut -d'.' -f2)
gitPatch=$(echo "$vsGit" | cut -d'.' -f3)
#Major Update
if((opiMajor != gitMajor)); then
        if(($month != $(date +%m))); then
                if(($(date +%H) > 8));then
                        echo "curl -s https://raw.githubusercontent.com/tonmai0705/innit-OrangePi-Loom-Update/refs/heads/Develop/update-opi-loom-majorUpdate.sh | bash"
                        newDate=$(date +%Y%m%d)
...
#Minor Update
elif((opiMinor != gitMinor)); then
        if(($day != $(date +%d))); then
                if(($(date +%H) > 8));then
                        echo "curl -s https://raw.githubusercontent.com/tonmai0705/innit-OrangePi-Loom-Update/refs/heads/Develop/update-opi-loom-minorUpdate.sh | bash"
                        newDate=$(date +%Y%m%d)
                        jq --arg val1 "$newDate" --arg val2 "$vsGit" '.date = $val1 | .version = $val2' "$updJson" > "$updJsontmp"
                        mv "$updJsontmp" "$updJson"
                        exit 0
...
#Patch Update
elif((opiPatch != gitPatch)); then
                        echo "curl -s https://raw.githubusercontent.com/tonmai0705/innit-OrangePi-Loom-Update/refs/heads/Develop/update-opi-loom-patchUpdate.sh | bash"
                        newDate=$(date +%Y%m%d)
                        jq --arg val1 "$newDate" --arg val2 "$vsGit" '.date = $val1 | .version = $val2' "$updJson" > "$updJsontmp"
                        mv "$updJsontmp" "$updJson"
                        exit 0
